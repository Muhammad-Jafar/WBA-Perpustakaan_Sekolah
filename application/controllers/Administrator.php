...
class Administrator extends CI_Controller {
    ...
    public function edit($id) {
        if ( $_SERVER['REQUEST_METHOD'] == 'POST') {
            $id = $this->security->xss_clean( $this->input->post('id_admin'));
            $level_user = $this->security->xss_clean( $this->input->post('level_user'));
            $nama_admin = $this->security->xss_clean( $this->input->post('nama_admin'));
            $username = $this->security->xss_clean( $this->input->post('username'));
            $password = $this->security->xss_clean( $this->input->post('password'));
            ...
            //terakhir 
            $this->m_admin->admin_update($level_user, $nama_admin, $username, $password, $avatar);
            redirect(base_url('administrator'));
        }

        $data = generate_page ('Edit data admin', 'administrator/edit/'. $id, 'Admin');
        $data_content['title_page'] = 'Edit data admin / pengelola';
        $data_content['admin'] = $this->m_admin->get_data_admin($id);
