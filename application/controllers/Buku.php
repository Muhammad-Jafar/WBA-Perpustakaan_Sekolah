...
class Buku extends CI_Controller {
	...
	public function data_buku() {
		$data = generate_page('Data Buku', 'buku/data_buku', 'Admin');
	...
	public function jenis_buku() {
		$data = generate_page('Data Buku', 'buku/jenis_buku', 'Admin');
	...
	public function add_new() {
		...
		switch ($name) {
			case 'jenis_buku' :
				if ( $_SERVER['REQUEST_METHOD'] == 'POST') {
					...
					if (!$this-> form_validation->run()) {
						...
						redirect(base_url('buku/add_new/'. $name));
					}

					$this->m_manajemenbuku->jenis_buku_add_new( $jenis_buku);
					redirect(base_url('buku/' . $name));
				}

				$data = generate_page ('Tambah data jenis buku', 'buku/add_new/' . $name, 'Admin');
			...
			case 'data_buku' : 
				...
				$data = generate_page ('Tambah data buku', 'buku/add_new/data_buku', 'Admin');
		...
		switch ($name) {
			case 'jenis_buku' :
				if ( $_SERVER['REQUEST_METHOD'] == 'POST') {
					...
					if (!$this-> form_validation->run()) {
						$this->session->set_flashdata('msg_alert', validation_errors());
						redirect(base_url('buku/edit/'. $name . '/' . $id) );
					}
				...
				$data = generate_page ('Ubah data jenis buku', 'buku/edit/'. $name . '/' . $id, 'Admin');
			...
			case 'data_buku' : 
				...
				$data = generate_page ('Ubah data buku', 'buku/edit/' . $name . '/' . $id, 'Admin');
